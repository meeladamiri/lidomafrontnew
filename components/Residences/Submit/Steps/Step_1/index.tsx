...
import { useSaveStep, useWizardOptions } from "../../useWizard";

/**
 * Step 1 — the kind of place.
 *
 * The first request the wizard makes is the one that creates the residence, so
 * this step genuinely has to wait for the server before it can move on: every
 * later step is keyed by the id it returns. What it no longer does is wait in
 * silence.
 */
function Step1() {
  const options = useWizardOptions("RES_TYPE", 1);
